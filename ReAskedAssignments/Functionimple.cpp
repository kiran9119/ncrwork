...
by the function*/
char * StringConcat(string str1, string str2,char* res)
{
	int i = 0,res_count=0;//result i
	while (str1[i] != '\0')
	{
		res[i] = str1[i];
		i++;
...
bool StringComp(string str1, string str2)
{
	bool flag = 0;
	int j = 0;
	while (str1[j]!='\0'||str2[j]!='\0')//traverse until any one of the string reaches the end
	{
		if (str1[j] != str2[j])
		{
			flag = 1;
	...
	if (str2[j]=='\0'&&str1[j]=='\0'&&flag==0)//if both the strings reach the end and all the elements are equal
		return 1;
	else
		return 0;
}
